...
TCP 紧急机制的目的是允许发送者优先接收者接收一些紧急数据，并允许接收 TCP 告知接收者，用户何时已经接收到所有当前已知的紧急数据。
...
the window is zero. This retransmission is essential to guarantee that when either TCP has a zero window the re-opening of the window
