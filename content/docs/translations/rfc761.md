...
{{< indent >}} 这个数表示 TCP 头部的长度有多少个 32 bit，表示真正数据开始的位置。TCP 头部（包括选项部分）的长度是 32 bit 的整数倍。
...
{{< indent >}} While LBB < SEG.SEQ+SEG.LEN  
{{< indent >}} Do LBB <- LBB + BS End  
{{< indent >}} SN <- LBB
...
{{< indent >}} foreign socket,  
{{< indent >}} 外部套接字,

{{< indent >}} local connection name,  
{{< indent >}} 本地连接名,

{{< indent >}} receive window,  
{{< indent >}} 接收窗口,

{{< indent >}} send window,  
{{< indent >}} 发送窗口,

{{< indent >}} connection state,  
{{< indent >}} 连接状态,

{{< indent >}} number of buffers awaiting acknowledgment,  
{{< indent >}} 等待确认的缓冲区数量,

{{< indent >}} number of buffers pending receipt (including partial ones),  
{{< indent >}} 等待接收的缓冲区的数量（包括部分缓冲区）,

{{< indent >}} receive buffer size,  
{{< indent >}} 接收缓冲区大小,
...
{{< indent >}} security/compartment,  
{{< indent >}} 安全/区段,

{{< indent >}} and default transmission timeout.  
{{< indent >}} 以及默认的传输超时,
...
{{< indent >}} dest = destination address  
{{< indent >}} dest = 目标地址

{{< indent >}} TOS = type of service  
{{< indent >}} TOS = 服务类型

{{< indent >}} TTL = time to live  
{{< indent >}} TTL = 存活时间

{{< indent >}} BufPTR = buffer pointer  
{{< indent >}} BufPTR = 缓存区指针

{{< indent >}} len = length of buffer  
{{< indent >}} len = 缓冲区长度

{{< indent >}} Id = Identifier  
{{< indent >}} Id = 标识符

{{< indent >}} DF = Don't Fragment  
{{< indent >}} DF = 不要分段

{{< indent >}} options = internet option data  
{{< indent >}} options = 网络选项数据

{{< indent >}} result = response  
{{< indent >}} result = 回应

{{< indent >}} OK = datagram sent ok
{{< indent >}} OK = 数据报已发送完毕

{{< indent >}} Error = error in arguments or local network error  
{{< indent >}} Error = 参数错误或本地网络错误
...
{{< indent >}} OK = datagram received ok  
{{< indent >}} OK = 数据报接收成功

{{< indent >}} Error = error in arguments  
{{< indent >}} Error = 参数错误

{{< indent >}} source = source address  
{{< indent >}} source = 源地址

{{< indent >}} dest = destination address  
{{< indent >}} dest = 目标地址

{{< indent >}} prot = protocol  
{{< indent >}} prot = 协议
...
{{< indent >}} SEGMENT ARRIVES

Timeouts 超时  
{{< indent >}} USER TIMEOUT  
{{< indent >}} RETRANSMISSION TIMEOUT
...
请注意，下面所有关于序列号、确认号、窗口等的算术都是以序列号空间的大小 2\*\*32 为模数。
...
if EOL = 0 then  
{{< indent >}} SND.NXT <- SEG.SEQ + SEG.LEN

if EOL = 1 then  
{{< indent >}} While SND.LBB < SEG.SEQ + SEG.LEN  
{{< indent >}} Do SND.LBB <- SND.LBB + SND.BS End  
{{< indent >}} SND.NXT <- SND.LBB
...
由于远程端已经发送了 FIN，RECEIVE 必须满足已经重新组合但尚未交付给用户的内容。
...
现在 RCV.NXT 和 RCV.LBB 设置为 SEG.SEQ+1，IRS 设置为 SEG.SEQ，任何其他操作或内容都应入队等待稍后处理。
...
注意，任何其它收到的操作或数据（与 SYN 关联）将在 SYN-RECEIVED 状态下处理，但 SYN 和 ACK 的处理不能重复。
...
如果段中还有其他操作或内容，则继续下面第五步检查 URG 位的处理，否则返回。
