...
TCP 紧急机制的目的是允许发送者鼓励接收者接收一些紧急数据，并允许接收 TCP 告知接收者，用户何时已经接收到所有当前已知的紧急数据。
...
格式：OPEN（本地端口，外部套接字，主动/被动 [，缓冲区大小] [，超时] [，优先级] [，安全/间隔]）-> 本地连接名称
...
TCP 或操作系统的某些组件会验证用户是否有权以指定的优先级或安全/间隔打开连接。
...
在 OPEN 调用中没有优先级或安全/间隔规范，表明应该使用默认值。
...
只有当安全/间隔信息完全相同且优先级等于或高于 OPEN 调用中请求的优先级时，TCP 才会匹配收到请求。
...
紧急特性的目的是鼓励接收方接收一些紧急数据，并向接收方指示何时已经收到当前所有已知的紧急数据。
