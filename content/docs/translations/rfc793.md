...
{{< indent >}} If SYN is present the sequence number is the initial sequence number (ISN) and the first data octet is ISN+1. {{< /indent >}}  
{{< indent >}} 如果存在 SYN，则序列号是初始序列号 (ISN)，第一个字节数据是 ISN+1。 {{< /indent >}}
...
**Data Offset**: 4 bits  
{{< indent >}} The number of 32 bit words in the TCP Header. This indicates where the data begins. The TCP header (even one including options) is an integral number of 32 bits long. {{< /indent >}}  
{{< indent >}} 这个数表示 TCP 头部的长度有多少个 32 bit，表示真正数据开始的位置。TCP 头部（即使包括选项部分）的长度是 32 bit 的整数倍。 {{< /indent >}}
...
{{< indent >}} PSH: Push Function 推送标志 {{< /indent >}}  
...
{{< indent >}} The TCP Length is the TCP header plus the data length in octets (this is not an explicitly transmitted quantity, but is computed from the total length, and the header length), and it does not count the 12 octets of the pseudo header. {{< /indent >}}  
{{< indent >}} TCP 长度是 TCP 报头加上数据部分的长度，单位是字节（这不是一个明确的传输量，而是从总长度和头部长度计算出来的），并且不包括 12 字节的伪头部。 {{< /indent >}}
...
{{< indent >}} number of buffers pending receipt, {{< /indent >}}  
{{< indent >}} 等待接收的缓冲区的数量, {{< /indent >}}
...
{{< indent >}} TIME-WAIT TIMEOUT {{< /indent >}}
