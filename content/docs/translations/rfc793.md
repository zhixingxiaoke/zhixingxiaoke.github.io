...
总结一下：每个发出的段在序列空间中占据一个或多个序列号，段所占据的序列号是 "忙" 或 "使用中"，直到 MSL 秒过去，崩溃时，一个时空块被最后发送的段的数据占据，如果新连接启动得太早，并且使用了前一个连接实例的最后一段时空足迹中的任何序列号，则存在潜在的序列号重叠区域，这可能会导致接收方混淆。
...
用户应该用 CLOSE 来回应，在此基础上，TCP 可以向另一端 TCP 发送 FIN。
